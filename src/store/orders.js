...
export const useOrdersStore = defineStore({
  ...
  getters: {
    ...
    getOrder: (state) => (id) => {
      let index = state.orders.findIndex(o => o.id == id)
  ...
  actions: {
    ...
    async getMyOrdersDB() {
        ...
        const userStore = useUserStore()
				const response = await fetch(`http://daw.deei.fct.ualg.pt/~a73824/LAB11/api/orders.php?session_id=${userStore.getUser.session_id}`)
